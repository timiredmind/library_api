...
class BookSchema(Schema):
    ...
    author_ = fields.Nested(
        AuthorCollectionSchema(
            only=["name"]),
        attribute="author",
        dump_only=True)
    publisher_ = fields.Nested(
        PublisherCollectionSchema(
            exclude=["id"]),
        attribute="publisher",
        dump_only=True)
    category_ = fields.Nested(
    ...
    loan_history = fields.Nested(
        BookLoanHistorySchema(
            many=True,
            exclude=["id"]),
        attribute="book_loan_history")
...
class UserSchema2(UserSchema):
    books_borrowed = fields.Nested(
        BooksBorrowedSchema(
            many=True,
            exclude=["id"]),
        attribute="books_borrowed")
...
class PaginatedCategorySchema(PaginationSchema):
    categories = fields.Nested(
        CategoryCollectionSchema(
            many=True), attribute="items")


class PaginatedPublisherSchema(PaginationSchema):
    publishers = fields.Nested(
        PublisherCollectionSchema(
            many=True), attribute="items")
